...
func (u *Topic) getCollectionName() string {
	return "topics"
}

func (u *Topic) First(filter bson.M, opts ...options.FindOptions) error {
	var (
...
func (u *Topic) FindById(id primitive.ObjectID) error {
	var (
		ctx, cancel = context.WithTimeout(context.Background(), 2*time.Second)
		db          = database.GetDB()
		filter      = bson.M{
...
func (u *Topic) Find(filter bson.M, opts ...*options.FindOptions) (Topics, error) {
	var (
		ctx, cancel = context.WithTimeout(context.Background(), 2*time.Second)
		db          = database.GetDB()
		topics      Topics
...
func (u *Topic) Update(filter bson.M, updateDoc bson.M, opts ...*options.UpdateOptions) error {
	var (
		ctx, cancel = context.WithTimeout(context.Background(), 2*time.Second)
		db          = database.GetDB()
		err         error
...
func (u *Topic) Delete(id primitive.ObjectID) error {
	var (
		ctx, cancel = context.WithTimeout(context.Background(), 2*time.Second)
		db          = database.GetDB()
		err         error
...
func (t *Topic) ParseEntry() bson.M {
	result := bson.M{
		"_id":         t.ID,
		"name":        t.Name,
